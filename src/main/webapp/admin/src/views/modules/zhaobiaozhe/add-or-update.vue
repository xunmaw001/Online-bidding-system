...
export default {
  data() {
	...
	zhaopian : false,
      ...
      rules: {
          zhaobiaozhanghao: [
                { required: true, message: '招标账号不能为空', trigger: 'blur' },
          ],
          mima: [
                { required: true, message: '密码不能为空', trigger: 'blur' },
          ],
          zhaobiaoxingming: [
                { required: true, message: '招标姓名不能为空', trigger: 'blur' },
          ],
          nianling: [
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `zhaobiaozhe/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.zhaobiaozheCrossAddOrUpdateFlag = false;
						 this.parent.search();
